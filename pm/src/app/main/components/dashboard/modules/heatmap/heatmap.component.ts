...
export class HeatmapComponent implements OnInit, AfterViewInit {
	...
	updateMarkers() {
		this.ms.getHeatmapData(this.selectedGroup.groupId).subscribe((res) => {
			...
			Object.keys(this.data).forEach((k) => {
				...
				LF.marker([ lat, lng ], { icon: this.phoneIcon }).addTo(this.layerGroup).bindPopup(`<b>Density: ${this
					.data[k].length}</b> <br>
                                                                <b>Area Code: ${k}</b>`);
			});
		});
	...
	ngOnInit() {
		this.phoneIcon = LF.icon({
			iconUrl: 'https://mercury-pm-images.s3.amazonaws.com/icons/phone_red_1.png',
