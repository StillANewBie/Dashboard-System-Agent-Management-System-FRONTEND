...
export class AddUserComponent implements OnInit {
	...
	submitUserInfo(form: FormGroup) {
		if (this.user) {
			...
			this.uas.saveUserInfo(ui, this.user.userId, true).subscribe((res) => {}, (err) => console.error(err));
			this.stepper.next();
		}
	}
	...
	submitGroupRole(form: FormGroup) {
		this.uas
			.saveUserRoleInfo(this.user.userId, this.groupRoleForm.get('role').value.id)
